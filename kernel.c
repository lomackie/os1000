...
void map_page(uint32_t *table1, uint32_t vaddr, paddr_t paddr, uint32_t flags) {
    ...
    if ((table1[vpn1] & PAGE_V) == 0) {
        // Create 1st level page table if it doesn't exist
...
void kernel_entry(void) {
    __asm__ __volatile__(
        // Retrieve kernel stack from sscratch
...
void kernel_main(void) {
    ...
    struct process *proc_a = create_process((uint32_t) proc_a_entry);
    struct process *proc_b = create_process((uint32_t) proc_b_entry);
